...
void LBFRegressor::GetCodefromRandomForest(struct feature_node *binfeature,
	...
	for (int iter = 0;iter<rand_forest.size();iter++){
		...
		for(int i = 0;i<rand_forest[iter].max_depth_-1;i++){
			...
			*/
			double project_x1 = rotation(0,0) * x1 + rotation(1,0) * y1;
			double project_y1 = rotation(0,1) * x1 + rotation(1,1) * y1;
			...
			*/
			double project_x2 = rotation(0,0) * x2 + rotation(1,0) * y2;
			double project_y2 = rotation(0,1) * x2 + rotation(1,1) * y2;
